# Collections
Collections, although not strictly the same, are akin to Categories. They serve to allow you to group products, either explicitly or via certain criteria, for use on your store.
...
| `match_conditions` | enum  | no | Available values are `['all', 'any']` |
...
:::tip
Models are customizable, and we recommend changing the **Collection** model when you configure your store. To change the model you need to look at the configuration file `config/shopper/models.php`.
:::

Let's keep in mind the modification that was made in the previous section regarding [Categories](/docs/{{version}}/categories).
...
return [
  'models' => [
    ...
    */
    ...
    'collection'  => \Shopper\Core\Models\Collection::class, // [tl! focus]
  ]
];
```
...
- Update `Collection` key for the model on the `models.php` config file to use our new model
  ```php
  return [
    'models' => [
      /*
...
## Manage Collections
From your Shopper admin on the sidebar go to **Collections**. The display page is rendered by the Livewire component `Shopper\Http\Livewire\Components\Collections\Browse` and for the display of the collections table is the component `Shopper\Http\Livewire\Tables\CollectionsTable`.

By default you will see this page without data which is rendered by a blade component of Shopper called [empty state](/docs/{{version}}/extending/empty-state).
...
### Create collection
Click on the **"Add new collection"** button on the collections page, and a creation form appears.
...
The Livewire component for collection creation is `Shopper\Http\Livewire\Collections\Create`. Here is the function to save a collection
...
To view the image of a collection you can consult the [media documentation](/docs/{{version}}/media#retrieving-images). And you can display collections in your frontend.
