...
The model used is `Shopper\Core\Models\Legal`. It uses the `HasSlug` trait for automatic slug generation.
...
`Legal::enabled()` is a scope that filters legal pages where `is_enabled` is `true`. This ensures draft pages are not shown to customers.
