...
| `allow_backorder` | boolean | no | false | Allow backorders |
...
For the full events reference including payload details and usage examples, see the [Events](/v2/events#product-events) page.
...
Following the same pattern as Laravel's `Model::preventLazyLoading()`, you can opt-in to catch
...
This helps you catch N+1 stock queries early. The exception message tells you exactly which model
and method to use:
...
Prices are managed through the polymorphic `prices` relationship. Each product can have multiple prices, one per currency. See the [Pricing documentation](/v2/pricing) for details.
