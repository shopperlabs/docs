...
![Payment architecture showing how a payment flows from the Storefront through Shopper to the Payment Provider and back](/images/shared/payment-architecture.png)
...
![Checkout Authorize flow: customer places order, Shopper initiates payment with capture_method manual, provider holds funds, transaction recorded as authorized](/images/shared/checkout-autorize.png)
...
![Capture flow: merchant clicks Capture on order detail, Shopper calls capturePayment on the provider, funds are transferred, transaction recorded as captured and order marked as Paid](/images/shared/capture.png)
